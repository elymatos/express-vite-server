...
function requireBytes() {
  ...
  bytesExports.format = format2;
  bytesExports.parse = parse2;
...
(function(exports2) {
  ...
  exports2.setCharset = function setCharset2(type2, charset3) {
    if (!type2 || !charset3) {
      return type2;
    }
...
res.clearCookie = function clearCookie(name, options) {
  var opts = merge2({ expires: new Date(1), path: "/" }, options);
...
function requireServeStatic() {
  ...
  serveStaticExports.mime = send4.mime;
 ...
 */
(function(module2) {
  module2.exports = expressExports;
})(express$2);
const express = /* @__PURE__ */ getDefaultExportFromCjs(expressExports$1);
const uuid = () => Date.now().toString(16) + Math.floor((1 + Math.random()) * 65536).toString(16);
var Role = /* @__PURE__ */ ((Role2) => {
  Role2[Role2["user"] = 0] = "user";
  Role2[Role2["staff"] = 1] = "staff";
  Role2[Role2["admin"] = 2] = "admin";
  Role2[Role2["owner"] = 3] = "owner";
  return Role2;
})(Role || {});
const users = [
  {
...
const tokens = [];
const router = expressExports$1.Router();
router.get("/", (req2, res2) => {
  if (req2.query.type === "shortcut") {
    return res2.send([
      req2.can(Role.staff) && {
      ...
      req2.can(Role.staff) && {
        ...
        children: [
          ...
          req2.can(Role.admin) && { id: "610159148224d543ae4e05b8", label: "User", name: "users" }
        ].filter(Boolean)
      }
    ].filter(Boolean));
  }
  res2.send([
    {
      ...
      children: [
        ...
        req2.can(Role.owner) && { id: "610159148224d543ae4e05c7", label: "Update", name: "update" }
      ].filter(Boolean)
    },
    req2.can(Role.staff) && {
...
app$1.request.can = function(role) {
  return this.user != null && this.user.role >= role;
};
app$1.use((req2, res2, next) => {
  var _a;
  if (req2.path.startsWith("/auth"))
    return next();
  const [, accessToken] = ((_a = req2.headers.authorization) == null ? void 0 : _a.split(" ")) ?? [];
  if (accessToken == null) {
    return res2.status(401).send({ message: "Requires authentication" });
  }
  const token = tokens.find((t) => t.access === accessToken);
  if (token == null) {
    return res2.status(401).send({ message: "Bad credentials: invalid access_token" });
  }
  if (token.expires <= Date.now()) {
    return res2.status(401).send({ message: "Bad credentials: access_token expired" });
  }
...
app$1.use("/menus", router);
app$1.use((req2, res2) => res2.status(404).json({ message: "Not Found" }));
const app = express();
app.get("/", (req2, res2) => {
  res2.send("change me to see updates llll, express~!");
});
app.get("/ip", async (req2, res2) => {
